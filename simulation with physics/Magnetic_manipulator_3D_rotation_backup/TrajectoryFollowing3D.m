...
CdTorque=0.00000005; 
RealCd=0.00015; %0.00015[N.s/m] Real Drag coefficient of the robot
Rho = 1000; %[Kg/m^3]
...
%////////////////////////////////////////////////////////////////////////
        ...
        set(gcf, 'Position', PlotPosition)
        hold on 
        HRobotOriC = quiver3(P(1)*1000,P(2)*1000,P(3)*1000,5*x,5*y,5*z,'color','g');
        HClosestPt = plot3(P(1)*1000,P(2)*1000,P(3)*1000,'*r');
        HPlotTraj = plot3(PositionHistory(:,1)*1000,PositionHistory(:,2)*1000,PositionHistory(:,3)*1000,'LineWidth',1,'color','r');
        HRobotToClosest = plot3([P(1)*1000,P(1)*1000],[P(2)*1000,P(2)*1000],[P(3)*1000,P(3)*1000],'y');
...
%         zlabel('z')
        ...
        grid on    
        hold on
        HRobotOri = quiver3(0,0,0,5*x,5*y,5*z,'color','g');
        HToClosestPt = quiver3(0,0,0,10*x,10*y,10*z,'color','r');
        HTrajOri = quiver3(0,0,0,0,0,0,'color','b','LineWidth',1);

        p2 = patch(robot,'EdgeColor','none','FaceColor',[0,0,0],'FaceLighting','phong','EdgeLighting','phong','FaceAlpha',0.6);
        P2OrigVert = p2.Vertices';
        R_X=[1,0,0;0,cos(pitch),-sin(pitch);0,sin(pitch),cos(pitch)];
        R_Y = [cos(roll),0,sin(roll);0,1,0;-sin(roll),0,cos(roll)];
        R_Z=[cos(yaw-pi/2),-sin(yaw-pi/2),0;sin(yaw-pi/2),cos(yaw-pi/2),0;0,0,1];
        p2.Vertices=(R_Z*R_X*R_Y*P2OrigVert)'+ 0.*repmat([1,0,0],[size(robot,1),1])+0.*repmat([0,1,0],[size(robot,1),1])+0.*repmat([0,0,1],[size(robot,1),1]);
        axis([-5,5,-5,5,-5,5])
...
%         plot3(Position(:,1)*1000,Position(:,2)*1000,Position(:,3)*1000,'LineWidth',1)
        set(HPlotTraj,'XData',Position(:,1)*1000,'YData',Position(:,2)*1000,'ZData',Position(:,3)*1000)
        set(HClosestPt,'XData', Ztraj(MinimumDistanceIndex),'YData', Xtraj(MinimumDistanceIndex), 'ZData', Ytraj(MinimumDistanceIndex))
%         plot3(Ztraj(MinimumDistanceIndex),Xtraj(MinimumDistanceIndex),Ytraj(MinimumDistanceIndex),'*r')
        set(HRobotToClosest,'XData',[P(1)*1000,Ztraj(MinimumDistanceIndex)],'YData',[P(2)*1000,Xtraj(MinimumDistanceIndex)],'ZData',[P(3)*1000,Ytraj(MinimumDistanceIndex)] )
%         plot3([P(1)*1000,Ztraj(MinimumDistanceIndex)],[P(2)*1000,Xtraj(MinimumDistanceIndex)],[P(3)*1000,Ytraj(MinimumDistanceIndex)],'y')
        set(HRobotOriC,'XData', P(1)*1000,'YData', P(2)*1000, 'ZData', P(3)*1000,'UData',5*x,'VData',5*y,'WData',5*z )
...
%         Current_direct = Current_direct./norm(Current_direct);
        ...
        hold on
        set(HRobotOri, 'UData',7.5*x,'VData',7.5*y,'WData',7.5*z); %   quiver3(0,0,0,5*x,5*y,5*z,'color','g')
%         quiver3(0,0,0,10*Current_direct(1),10*Current_direct(2),10*Current_direct(3),'color','r')
        set(HTrajOri, 'UData',10*orientation(MinimumDistanceIndex,3),'VData',10*orientation(MinimumDistanceIndex,1),'WData',10*orientation(MinimumDistanceIndex,2)); 
%         quiver3(0,0,0,10*orientation(MinimumDistanceIndex,3),10*orientation(MinimumDistanceIndex,1),10*orientation(MinimumDistanceIndex,2),'color','b','LineWidth',1)
        set(HToClosestPt, 'UData',7.5*TargetPointDierection(1),'VData',7.5*TargetPointDierection(2),'WData',7.5*TargetPointDierection(3));         
