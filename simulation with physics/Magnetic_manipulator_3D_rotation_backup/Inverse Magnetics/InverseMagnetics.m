function [I,Act] = InverseMagnetics(TotalForce,Orien,P,coil_radius,MagneticMoment,T,MaxI,NbTurns,IPrevious,RobotMass,V)
...
minOmega = 300;
maxOmega = 750;
Kp = 10; %Velocity PID paprameter Kp
% disp ('Omega from force PID')
% disp( norm(TotalForce)/ThrustCoe )
% disp('Velocity in inverse')
% disp(norm(V))

%velocity PID controller
ErrorV = Vsetpoint - norm(V);
if ErrorV > 0
    if norm(V)==0
        K =1+ ErrorV;
    else
...
% disp("Kp")
% disp(Kp)
% disp(Kp*norm(TotalForce)/ThrustCoe)
%map force to rotation frequency
NewOmega = K*(norm(TotalForce)/ThrustCoe);

if NewOmega > maxOmega
    TotalForce = (maxOmega/minOmega*RobotMass) .*(TotalForce./norm(TotalForce));
elseif NewOmega < minOmega
    TotalForce = RobotMass .*(TotalForce./norm(TotalForce));
else
    TotalForce = ((NewOmega/minOmega) * RobotMass) .*(TotalForce./norm(TotalForce));
end

disp('Omega from velocity PID')
disp(norm(TotalForce)/RobotMass*minOmega)

disp("Total force pafter mapping")
disp(TotalForce)
%Outputs: 
