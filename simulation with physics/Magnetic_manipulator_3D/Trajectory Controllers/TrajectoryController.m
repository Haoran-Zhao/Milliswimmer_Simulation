function [TotalForce,Orien,MinimumDistanceIndex,IntegralOut] = TrajectoryController(P, Traj,orientation,Kp,Ki, MaxVelocity,RobotMass,Robot_diameter,CdForce,Rho,Integral,deltaT)
[m,~]=size(Traj);
...
FPID=Kp.*ErrorPosition+Ki.*Integral; % force asked by PID controller [N]

%Calculate total force to apply to the robot
Weight = RobotMass.*9.81.*[0 0 1];
TotalForce=FdTargetVec+FPID+Weight;
% ForceMagnitude=norm(TotalForce);
%Calculate orientation
Orien = TotalForce./norm(TotalForce);
