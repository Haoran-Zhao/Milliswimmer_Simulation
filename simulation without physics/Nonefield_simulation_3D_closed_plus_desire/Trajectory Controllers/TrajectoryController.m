...
while RobotDistanceFromPath(MinimumDistanceIndex)<5
    ...
    if MinimumDistanceIndex < 1000
        MinimumDistanceIndex = MinimumDistanceIndex+4;
    if MinimumDistanceIndex >=1000
        MinimumDistanceIndex=MinimumDistanceIndex-length(VelocityHighResTrajectory)+4;
    end
    end
...
if (MinimumDistanceIndex)>length(VelocityHighResTrajectory)
    ...
    TargetPointw=orientation(MinimumDistanceIndex-length(VelocityHighResTrajectory),1);
    TargetPointu=orientation(MinimumDistanceIndex-length(VelocityHighResTrajectory),2);
    TargetPointv=orientation(MinimumDistanceIndex-length(VelocityHighResTrajectory),3);
...
TargetPointVector = -[P(1)*1000,P(2)*1000,P(3)*1000]+[Ztraj(MinimumDistanceIndex),Xtraj(MinimumDistanceIndex),Ytraj(MinimumDistanceIndex)]+[TargetPointw,TargetPointu,TargetPointv];
...
%     point to the closed or ahead point
    VPID = MaxVelocity.*TargetPointDierection;
elseif norm(VPID) < ratio*MaxVelocity
    VPID = ratio*MaxVelocity.*TargetPointDierection;
else
    VPID = norm(VPID).*TargetPointDierection;
end
Velocity = VPID;
